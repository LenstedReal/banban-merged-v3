        ...
        if found:
            ...
            if event_id:
                try:
                    ...
                    if r.status_code == 200:
                        md = r.json()
                        if md.get("Tr1") is not None: stats["score"]["home"] = md.get("Tr1")
                        if md.get("Tr2") is not None: stats["score"]["away"] = md.get("Tr2")
                        ...
                        if md.get("Esd"): stats["start_date"] = md.get("Esd")
                ...
                try:
                    ...
                    if stat_r.status_code == 200:
                        ...
                        home_stat, away_stat = {}, {}
                        for s in stat_list:
                            if s.get("Tnb") == 1: home_stat = s
                            elif s.get("Tnb") == 2: away_stat = s
                        stat_map = {
                            "ball_possession":   ("Pss", "%"),
                        ...
                        for stat_key, (field, suffix) in stat_map.items():
                            hv = home_stat.get(field, 0); av = away_stat.get(field, 0)
                            if hv or av:
            ...
            GOAL_ITS = {4, 36, 49, 50}; PEN_ITS = {37}; OG_ITS = {38}
            YC_ITS = {6, 39}; SYC_ITS = {41}; RC_ITS = {7, 40}; SUB_ITS = {13, 26}
            seen = set()
            for side, inc in md_incs:
                it = inc.get("IT")
                key = (side, it, inc.get("Min"), inc.get("Pn") or inc.get("Player"))
                if key in seen: continue
                seen.add(key)
                kind = label = None
                if it in YC_ITS: kind, label = "yellow", "SARI KART"
                elif it in SYC_ITS: kind, label = "red", "2. SARI = KIRMIZI"
                elif it in RC_ITS: kind, label = "red", "KIRMIZI KART"
                elif it in SUB_ITS: kind, label = "sub", "DEĞİŞİKLİK"
                elif it in PEN_ITS: kind, label = "goal", "PENALTI GOL"
                elif it in OG_ITS: kind, label = "goal", "K. KALE GOL"
                elif it in GOAL_ITS: kind, label = "goal", "GOL"
                if kind:
                    player = inc.get("Pn") or (f"{inc.get('Fn','')} {inc.get('Ln','')}".strip()) or inc.get("Player") or "—"
                    stats["events"].append({
                    ...
                    for inc in (inc_r.json() or {}).get("incidents") or []:
                        ...
                        if itype == "substitution":
                            new_ev = {"minute": minute, "type": "sub", "label": "DEĞİŞİKLİK", "team": team,
                                      "player": (inc.get("playerIn") or {}).get("name") or "—",
                            ...
                            new_ev = {"minute": minute, "type": "goal", "label": "GOL", "team": team,
                                      "player": (inc.get("player") or {}).get("name") or "—",
                                      "assist": (inc.get("assist1") or {}).get("name") if inc.get("assist1") else None}
                        if new_ev and (new_ev["minute"], new_ev["type"], new_ev["team"]) not in existing:
                            stats["events"].append(new_ev)
                            existing.add((new_ev["minute"], new_ev["type"], new_ev["team"]))
